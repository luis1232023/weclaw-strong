...
type AgentConfig struct {
	Type         string            `json:"type"`                    // "acp", "cli", "http", or "serve"
	Command      string            `json:"command,omitempty"`       // binary path (cli/acp/serve type)
	...
	Env          map[string]string `json:"env,omitempty"`           // extra environment variables (cli/acp/serve type)
	...
	Port         int               `json:"port,omitempty"`          // listen port (serve type, default 8080)
	Host         string            `json:"host,omitempty"`          // listen host (serve type, default "127.0.0.1")
}
